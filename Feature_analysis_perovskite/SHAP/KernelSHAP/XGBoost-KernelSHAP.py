...
model = XGBRegressor(
    ...
    n_estimators=206,
    max_depth=6,
    learning_rate=0.09945134334872878,
    subsample=0.5009629706436092,
    colsample_bytree=0.9780611296739763,
    gamma=0.10137475884738077,
    reg_alpha=0.002237225276178238,
    reg_lambda=0.0005355340462282876
)
...
for train_index, val_index in kf.split(X):
    ...
    f = lambda X: model.predict(X)  # 包装预测函数，避免 SHAP 修改模型对象
    ...
    shap_values = explainer.shap_values(X_val,n_jobs=-1)  # nsamples 可调，越大越准但越慢
...
plt.savefig("XGBoost_KernelSHAP_custom_order.png", dpi=300, bbox_inches="tight")
plt.close()
...
ga_df.to_excel("GA_ranking_XGBoost_KernelSHAP.xlsx", index=False)
