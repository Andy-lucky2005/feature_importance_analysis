...
for i, (group_name, group_methods) in enumerate(method_groups.items()):
    ...
    plt.yticks(y_pos, forest_df["feature"], fontsize=17)
    ...
    # 手动统一边距
    plt.subplots_adjust(left=0.35, right=0.95, top=0.97, bottom=0.22)
    plt.tight_layout(pad=0)
    # plt.savefig(f"forest_{group_name}.png", dpi=300, bbox_inches="tight", pad_inches=0)
    plt.savefig(f"forest_{group_name}.pdf", dpi=300, bbox_inches="tight", pad_inches=0)
    plt.close()